...
コンテナレイヤーはコンテナが破棄されるときに削除される。

### 永続化
### ボリューム
Dockerが管理しているボリューム領域を利用することで永続化できる
...
マウントポイントが確認できる
ただし仮想的にlinux環境を利用しているので、`cd`とかでアクセスすることはできない。windowsも同じ、ubuntuはアクセスできるはず。

`docker container run -v [volume名]:[コンテナ内の絶対パス] [イメージ]`
### バインドマウント
dockerが管理する領域にボリューム領域を作成するのではなく、ローカル環境とコンテナで記憶領域を共有する。
`docker container run -v [ホスト絶対パス]:[コンテナ内絶対パス] [イメージ]`
### ボリュームとバインドマウントの使い分け
ボリューム、マウント：複数のコンテナで共有可能
...
Docker公式ではボリュームを推奨している。
ホスト側の中身によってコンテナに副作用が起きるので、ホスト側からアクセスする必要がない場合はボリュームを利用する。
...
docker networkを利用しているときは削除できないので
`-f`を付ける必要がある。

ネットワークは名前解決できるので`curl http://my-net`などが利用できる。もちろん、他のネットワークとはつながってない。
